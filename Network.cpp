...
double Network::calculate_cost(vector<double> &input_arr, int actual, vector<vector<double>>& delta_biases, vector<vector<vector<double>>>& delta_weights) {
    vector<double> hidden_arr(num_hidden_neurons, 0.0);
    vector<double> hidden_arr_z(num_hidden_neurons, 0.0);
    vector<double> res_z(neurons[2], 0.0);
    vector<double> res(neurons[2], 0.0);

    this->feed_forward(input_arr, hidden_arr_z, 0);
    for (int i = 0; i < num_hidden_neurons; i++) hidden_arr[i] = sigmoid(hidden_arr_z[i]);

    this->feed_forward(hidden_arr, res_z, 1);
    for (int i = 0; i < neurons[2]; i++) res[i] = sigmoid(res_z[i]);
    ...
    for (int j = 0; j < neurons[num_layers - 1]; j++) {
        double temp = (j == actual ? (res[j] - 1.0) : (res[j])) * 2 * sigmoid_prime(res_z[j]);
        delta[j] = temp;
        for (int k = 0; k < neurons[num_layers - 2]; k++) {
            delta_weights[num_layers - 2][j][k] += temp * hidden_arr[k];
        }
    ...
    for (int j = 0; j < neurons[num_layers - 2]; j++) {
        ...
        for (int k = 0; k < neurons[num_layers - 3]; k++) {
            delta_weights[num_layers - 3][j][k] += temp * input_arr[k];
        }
...
void Network::apply(vector<vector<double>>& delta_biases, vector<vector<vector<double>>>& delta_weights) {
    for (int i = 0; i < num_layers - 1; i++) {
        for (int j = 0; j < neurons[i + 1]; j++) {
            biases[i][j] -= learning_rate * (delta_biases[i][j] / train_size);
            for (int k = 0; k < neurons[i]; k++) {
                weights[i][j][k] -= learning_rate * (delta_weights[i][j][k] / train_size);
            }
        }
    }
}
...
int Network::evaluate() {
    ...
    for (int n = 0; n < label_number_of_items; n++) {
        ...
        this->feed_forward_sigmoid(hidden_arr, res, 1);
